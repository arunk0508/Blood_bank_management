...
<div class="head-bar-1">
    ...
    $q="select * from patientlist";
    $rl=mysqli_query($con,$q);

    echo"<table class='table'>";
    echo"<tr><td>S.No</td><td>Patient ID</td><td>Patient Name</td><td>Patient Email</td><td>Mobile No.</td><td>Action</td></tr>";
    while($row=mysqli_fetch_array($rl)){
        echo"<tr><td>$row[sno]</td><td> $row[patientid]</td><td> $row[patientname]</td> <td>$row[patientemail]</td><td> $row[mobileno]</td>";
