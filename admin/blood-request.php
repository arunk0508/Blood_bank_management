...
echo"<div class='list'>";
echo"<p class='list-of-donors'>Manage Blood Requests</p>";
echo"</div>";
echo"<div class='main-body'>";
    $con=mysqli_connect($servername,$username,$password,$database);
    $q="select * from adminbloodrequest";
    $rl=mysqli_query($con,$q);

    echo"<table class='table' style='margin-left:400px;'>";
    echo"<tr><td>S.No</td><td>Request ID</td><td>Patient Name</td><td>Mobile No</td><td>Blood group</td><td>Units(in ml)</td><td>Reason</td><td>Status</td><td>Action</td></tr>";
    while($row=mysqli_fetch_array($rl)){
        echo"<tr><td>$row[sno]</td><td> $row[requestid]</td><td> $row[patientname]</td><td>$row[mobileno]</td><td>$row[bloodgroup]</td><td>$row[unit]</td><td>$row[reason]</td><td>$row[status]</td>";
