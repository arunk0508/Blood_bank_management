...
<div class="head-bar-1">
   ...
   <br>
    <?php
        ...
        echo" <div class='list'>";
        echo"<p class='list-of-donors'>List of all Patients</p>";
    echo"</div>"; 

    echo"<div class='main-body'>";
    $con=mysqli_connect($servername,$username,$password,$database);
    $q="select * from donationrequest";
    $rl=mysqli_query($con,$q);

    echo"<table class='table' style='margin-left:400px;'>";
    echo"<tr><td>S.No</td><td>Donation ID</td><td>Donor Name</td><td>Mobile No</td><td>Blood group</td><td>Units(in ml)</td><td>Disease</td><td>Status</td><td>Action</td></tr>";
    while($row=mysqli_fetch_array($rl)){
        echo"<tr><td>$row[sno]</td><td> $row[donationid]</td><td> $row[donorname]</td><td>$row[mobileno]</td><td>$row[bloodgroup]</td><td>$row[unit]</td><td>$row[disease]</td><td>$row[status]</td>";
